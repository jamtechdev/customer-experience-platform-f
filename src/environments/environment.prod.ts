/**
 * Sentimenter CX - Production Environment Configuration
...
export const environment = {
  ...
  cache: {
    ...
    ttl: getEnvNumber('NG_APP_CACHE_TTL', 600), // 10 minutes in production
    maxSize: getEnvNumber('NG_APP_CACHE_MAX_SIZE', 200), // max 200 cached items
  },
  
  // Analytics & Tracking
  analytics: {
    enabled: getEnvBoolean('NG_APP_ANALYTICS_ENABLED', true),
    googleAnalyticsId: getEnv('NG_APP_GOOGLE_ANALYTICS_ID', 'G-XXXXXXXXXX'), // Replace with actual GA4 ID
  },
  
  // Logging
  logging: {
    level: getEnv('NG_APP_LOG_LEVEL', 'error') as 'debug' | 'info' | 'warn' | 'error',
    console: getEnvBoolean('NG_APP_LOG_CONSOLE', false),
    remote: getEnvBoolean('NG_APP_LOG_REMOTE', true),
  },
